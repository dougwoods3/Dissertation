...
%Hypre, an open source parallelizable linear solver library \cite{Hypre_Web} is commonly used for calculations that use the parallel build of MFEM.

%It is capable of detecting and solving cycles within the mesh (i.e. when a complex mesh cell both contributes to the upwind flux of a neighboring cell and receives the upwind flux from that same cell on the other side).
